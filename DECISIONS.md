...
- **Amended 2026-09-10: `flag.reviewed` no longer emitted** (the emitted set is **18**). Unlike `evidence.added` above, this type is not deleted from the schema — 19 runs recorded before this date contain it, and `Event.payload()` has to keep validating against a known type for `runectl trace show`/`replay` to read them. See the D11/D15 amendments of the same date.
...
**Amended 2026-09-10 — the disconfirmation review is removed, not merely advisory.** An advisory check that never changes a decision is not a check, it is a line item: `_apply_policy` never read the review's verdict (only `format_ok` and `rederived` decide `gated`), so every one of the nine live reviews this file cites — six clears, two wrong-flags-cleared, one correct-flag-held, zero caught — spent ≈$0.002 and a provider round trip on a call whose output could not move any run's outcome. Kept as dead weight, it was also actively misleading: `flag list`/`trace show` printed a "review sound/doubtful" line next to every candidate that read as a judgment, when nothing downstream consulted it. `flags/review.py`, `FlagReviewed`/`flag.reviewed`, the `Reviewer` protocol, and every call site (`flags/judge.py`, `loop/runner.py`, `cli/run_cmd.py`, `cli/app.py`'s replay path) are deleted. `cli/render.py` keeps its `FlagReviewed` render branch, and `trace/events.py` keeps the `FlagReviewed` payload class, as **read-only**: 19 of the runs already on disk when this was written contain `flag.reviewed` events, and `runectl trace show` / `runectl replay` on any of them must keep working. Nothing emits the event going forward.

Verifying that last claim caught a second, unrelated bug worth recording here rather than in a footnote: `EventPayload`'s `extra="forbid"` meant `TraceReader` silently truncated *every* run recorded before 2026-09-10's dead-code pass the instant it hit a field that pass had removed (`artifact_ref`, `provenance_artifact`, `timed_out`) — `runectl trace show`/`replay` on any of them stopped mid-run with no error, which is exactly the silent behavior `TraceReader`'s own docstring says is reserved for real corruption (a SIGKILL mid-write), not an intentional schema edit. Fixed in the same commit: `extra="ignore"`. Swept all 47 runs on disk after the fix — every one now reads to its actual end except one held back by a separate, already-documented issue (the spilled-artifact-dict hazard noted in `loop/runner.py:61-71`, out of scope here).

A bench run before and after the review removal (`bench/results/2026-09-10-baseline-with-review.json`, `2026-09-10-no-review.json`) is the verification for the removal itself — same solve rate and same false-flag count, because the mechanism never had a vote; one fewer utility-model call per candidate and no more advisory line that read as a verdict but wasn't one.
