...
- **The arena grew a toolset (`arena/Dockerfile`).** The new playbooks name tools the image lacked, and `test_categories.py` enforces that every `required_tools` entry is installed. Added: `rizin`, `ltrace`, `strace`, `upx-ucl`, `one_gadget` (pwn/rev); `outguess`, `sleuthkit`, `poppler-utils`, `volatility3`, `sox`/`ffmpeg`/`multimon-ng`, `unrar` (forensics); `whois`, `dnsutils` (osint); `nmap`, `aircrack-ng`, `wireshark-common` for `capinfos` (network). This changes the Dockerfile fingerprint (D17), so an existing arena warns as stale until rebuilt — expected.
